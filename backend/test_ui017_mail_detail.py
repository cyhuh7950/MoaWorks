...
class FakeCursor:
    ...
    def execute(self, sql, params=()): self.executions.append((" ".join(str(sql).split()), tuple(params)))
    def fetchone(self): return self.fetchone_results.pop(0) if self.fetchone_results else None
    def fetchall(self): return self.fetchall_results.pop(0) if self.fetchall_results else []
...
class FakeDb:
    def __init__(self, *, fetchone=None, fetchall=None):
        ...
        self.ensure_count = 0
    def ensure_migrations_applied(self): self.ensure_count += 1
...
class MailDetailTest(unittest.TestCase):
    ...
    def test_access_query_scopes_sender_and_recipient_soft_delete_views(self):
        service = MailMessengerService(); cursor = FakeCursor(fetchone=[self.message(sender_view=False)])
        service._fetch_accessible_mail(cursor, self.actor(), "mail-1")
        sql, params = cursor.executions[0]; normalized = sql.upper()
        self.assertIn("M.SENDER_DELETED_AT IS NULL", normalized)
        self.assertIn("R.DELETED_AT IS NULL", normalized)
        self.assertNotIn("mail-1", sql); self.assertIn("mail-1", params)

    def test_recipient_visibility_keeps_to_cc_and_only_actor_bcc(self):
        service = MailMessengerService(); actor = self.actor("bcc-self", "self@example.test")
        rows = [self.recipient("to@example.test", "to", "to-user"), self.recipient("cc@example.test", "cc", "cc-user"), self.recipient("self@example.test", "bcc", "bcc-self")]
        cursor = FakeCursor(fetchall=[rows])
        recipients = service._fetch_mail_recipients(cursor, actor, "mail-1", is_sender_view=False)
        self.assertEqual([item.recipientEmail for item in recipients], ["to@example.test", "cc@example.test", "self@example.test"])
    ...
    def test_sender_visibility_keeps_all_bcc(self):
        ...
        rows = [self.recipient("to@example.test", "to", "to-user"), self.recipient("hidden-a@example.test", "bcc", "bcc-a"), self.recipient("hidden-b@example.test", "bcc", "bcc-b")]
        cursor = FakeCursor(fetchall=[rows])
    ...
    def test_route_keeps_mail_forbidden_403_contract(self):
        ...
        with patch.object(mail_routes, "_service", return_value=service):
            with self.assertRaises(HTTPException) as captured: mail_routes.get_mail("mail-deleted", self.actor())
