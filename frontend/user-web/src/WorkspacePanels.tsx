...
export function WorkspacePanels({ menu, token, locale, timezone, initialSelectionId, onPreferencesSaved }: Props) {
  ...
  function openSchedule(item?: WorkspaceSchedule) {
    ...
    setSelectedId(item?.id ?? ""); setScheduleEditingId(item?.id ?? null); setModal("schedule");
  }
  ...
  async function submitSchedule(event: FormEvent) {
    ...
    try { setError(""); await saveSchedule(token, { ...scheduleForm, startsAt: new Date(scheduleForm.startsAt).toISOString(), endsAt: new Date(scheduleForm.endsAt).toISOString() }, scheduleEditingId ?? undefined); setError(""); setNotice("일정을 저장했습니다."); setModal("none"); await refresh(); }
  ...
  if (menu === "schedule") view = <section style={{ display: "grid", gridTemplateColumns: "minmax(280px,.8fr) minmax(420px,1.2fr)", gap: 16, minHeight: 0, height: "100%" }}><article style={panelStyle}><header style={{ display: "flex", justifyContent: "space-between" }}><h2 style={{ margin: 0, fontSize: 22 }}>일정</h2><button type="button" onClick={() => openSchedule()} style={primaryButtonStyle}>일정 만들기</button></header><div style={{ marginTop: 12, display: "grid", gap: 6 }}>{schedules.map((item) => <button type="button" key={item.id} onClick={() => setSelectedId(item.id)} onDoubleClick={() => openSchedule(item)} style={{ ...buttonStyle, height: "auto", minHeight: 54, textAlign: "left", background: selectedId === item.id ? "#e6fffb" : "#fff" }}><strong>{item.title}</strong><div style={{ color: "#64748b", marginTop: 4 }}>{formatDate(item.starts_at)} - {formatDate(item.ends_at)}</div></button>)}</div></article>{selectedSchedule ? detail(selectedSchedule.title, [["시작", formatDate(selectedSchedule.starts_at)], ["종료", formatDate(selectedSchedule.ends_at)], ["메모", selectedSchedule.description]], () => openSchedule(selectedSchedule), () => setModal("confirm-delete")) : detail("선택 일정", [["안내", "일정을 선택하거나 새로 만드세요."]])}</section>;
