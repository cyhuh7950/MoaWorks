...
export function CalendarSettingsPanel({ token, data, onChanged, onBack }: Props) {
  ...
  return <main className="ui039-calendar-settings">
    ...
    {tab === "owned" ? <>
      <form className="ui039-add" onSubmit={add}><label><span>캘린더 이름</span><input required maxLength={32} value={name} onChange={(event) => setName(event.target.value)} /></label><label><span>색상</span><select value={color} onChange={(event) => setColor(event.target.value as typeof color)}>{calendarColors.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><button disabled={busy || !name.trim()}>캘린더 추가</button></form>
      <section className="ui039-owned-list">{data.owned.map((calendar, index) => <article key={calendar.id}>
        ...
        <label><span>이름</span><input defaultValue={calendar.name} maxLength={32} onBlur={(event) => event.target.value.trim() !== calendar.name && void patch(calendar, { name: event.target.value.trim() })} /></label>
      ...
      <section className="ui039-incoming"><h3>내 일정을 보고 있는 동료</h3>{data.incomingRequests.length ? data.incomingRequests.map((item) => <article key={item.subscriptionId}><span><strong>{item.subscriber.name}</strong><small>{item.subscriber.department}</small></span><span>{item.calendar.name}</span>{item.status === "pending" ? <div><button type="button" onClick={() => void act(() => decideCalendarSubscription(token, item.subscriptionId, "accept"))}>수락</button><button type="button" onClick={() => void act(() => decideCalendarSubscription(token, item.subscriptionId, "reject"))}>거절</button></div> : <strong>열람 중</strong>}</article>) : <p>연결된 동료가 없습니다.</p>}</section>
    </> : <>
