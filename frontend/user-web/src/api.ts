...
export async function fetchWorkspaceFiles(token: string, options: { scope?: WorkspaceFileScope; folderId?: string | null; query?: string; sort?: string } = {}) { const params = new URLSearchParams(); if (options.scope) params.set("scope", options.scope); if (options.folderId) params.set("folderId", options.folderId); if (options.query) params.set("query", options.query); if (options.sort) params.set("sort", options.sort); return request<{ items: WorkspaceFile[] }>(`/workspace/files${params.size ? `?${params}` : ""}`, { headers: authHeaders(token) }); }
export async function fetchWorkspaceFileDetail(token: string, id: string) { return request<WorkspaceFileDetail>(`/workspace/files/${id}`, { headers: authHeaders(token) }); }
