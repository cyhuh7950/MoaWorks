...
export function MessengerPanel({ token }: { token: string }) {
  ...
  return <section className="ui040-messenger">
    <aside className="ui040-room-list"><header><h2>메신저</h2><button className="is-primary" type="button" onClick={() => setCreateOpen(true)}>새 대화</button></header>
      ...
      <div className="ui040-room-scroll">{loading ? <p className="ui040-state">대화방을 불러오는 중입니다.</p> : filteredRooms.length === 0 ? <div className="ui040-state"><p>검색 결과가 없습니다.</p><button className="is-primary" type="button" onClick={() => setCreateOpen(true)}>새 대화 시작</button></div> : <>{roomGroup("즐겨찾기", favoriteRooms)}{roomGroup("최근 대화", recentRooms)}</>}</div>
    </aside>
