...
const checks = [
  ...
  ["운영 무결성 live 재확인", deployment.status === "PASS" && deployment.liveDeployment.status === "PASS" && deployment.liveDeployment.worktreeClean === true && deployment.liveDeployment.productRevision === manifest.git.productRevision && deployment.liveDeployment.webRuntime.userWeb === "PASS_NGINX_STATIC_NO_NODE_NPM_SOURCE" && deployment.liveDeployment.webRuntime.adminWeb === "PASS_NGINX_STATIC_NO_NODE_NPM_SOURCE" && deployment.liveDeployment.externalHttp.userRoot === 200 && deployment.liveDeployment.externalHttp.userHealth === 200 && deployment.liveDeployment.externalHttp.adminRoot === 200 && deployment.liveDeployment.externalHttp.adminHealth === 200 && deployment.liveDeployment.rollback.status === "PASS"],
  ...
  ["핵심 GAP 유지", result.gaps.length === 2 && result.gaps.includes("CHROME_RUN1_NOT_RUN") && result.gaps.includes("CHROME_RUN2_NOT_RUN")],
