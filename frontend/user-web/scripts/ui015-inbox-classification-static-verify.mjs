...
const checks = [
  ...
  ["분류 필터 접근성", app.includes('<select aria-label="분류 필터"')],
  ...
  ["성공 후 inbox 재조회", categoryFunction.includes("await fetchInbox(token, nextQuery)")],
  ["대상 분류 이동", categoryFunction.includes("setMailListQuery(nextQuery)")],
