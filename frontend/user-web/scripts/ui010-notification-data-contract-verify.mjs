...
const evidenceDir = resolve(root, "docs/evidence/ui010-notification-data-contract", runId);
const progressPath = resolve(evidenceDir, "progress.jsonl");
await mkdir(evidenceDir, { recursive: true });

const state = { adminToken: "", adminUserId: "", observerToken: "", observerUserId: "", roleIds: [], userIds: [], notificationIds: [], notificationBySuffix: {}, preferenceAuditIds: [], scheduleIds: [], originalPreferences: null };
const network = [];
const consoleErrors = [];
const httpErrors = [];
...
async function execDb(code) {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      return await execFileAsync("uv", ["run", "python", "-c", code], { cwd: backend, timeout: 30000 });
    } catch (error) {
      lastError = error;
...
async function request(path, options = {}) {
  ...
  try {
    ...
    throw new Error(`${method} ${path}: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function cleanup() {
  const errors = [];
  for (const id of state.scheduleIds) {
    const response = await request(`/workspace/schedules/${id}`, adminOptions("DELETE"));
    if (response.status < 200 || response.status >= 300) errors.push(`schedule:${id}:${response.status}`);
  }
...
try {
  ...
  const accessData = await step("prepare", async () => {
    ...
    const schedule = await ok("/workspace/schedules", adminOptions("POST", {
      title: `${runId}.schedule`, startsAt, endsAt, description: "UI-010 source navigation",
    }));
  ...
  await record("chrome-admin-login", "success");
    ...
    await recentDialog.getByRole("button").filter({ hasText: `${runId} schedule-link` }).click();
    await page.getByRole("heading", { name: "일정", exact: true }).waitFor();
    await page.getByText(`${runId}.schedule`, { exact: true }).first().waitFor();
    await record("chrome-admin-source-navigation", "success");

    await page.getByRole("button", { name: /알림 \d+건/ }).click();
    await page.getByRole("dialog", { name: "최근 알림" }).getByRole("button", { name: "모두 읽음", exact: true }).click();
    await page.getByRole("button", { name: "알림 0건", exact: true }).waitFor({ timeout: 10000 });
    await page.getByRole("dialog", { name: "최근 알림" }).getByRole("button", { name: "전체 알림 보기", exact: true }).click();
    await page.getByRole("heading", { name: "전체 알림" }).waitFor();
    await page.getByText(`${runId} schedule-link`, { exact: true }).waitFor();
    await record("chrome-admin-read-all", "success");

    const archiveRow = page.locator(".notification-center-row").filter({ hasText: `${runId} archive` }).first();
    await archiveRow.waitFor({ state: "visible", timeout: 10000 });
    await archiveRow.getByRole("checkbox").check();
  ...
  await record("chrome-admin-preferences", "success");
    ...
    result.checks.chrome = { recentPopup: true, sourceNavigation: true, readAllConsistency: true, softDelete: true, preferences: true, measurements };
    await context.close();
  }, 90000);
