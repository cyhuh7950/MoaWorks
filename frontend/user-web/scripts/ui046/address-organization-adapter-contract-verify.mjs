...
function cleanupFixture(records, overrides = {}) { return { runId, residualOwnedRows: 0, residualOwnedAudit: 0, sessionsClosed: true, existingRowChanges: 0, disposableIdentities: records.filter((x) => ["test_role", "test_user"].includes(x.kind)).map((x) => ({ kind: x.kind, id: x.id, ownerRunId: runId, active: false })), protectedAccounts: [fp("admin")], referenceDepartment: fp(ids.department), existingAddressOrganizationFingerprint: { before: "addr_org_fp", after: "addr_org_fp" }, ...overrides }; }
...
const existingProtectedOnly = cleanupFixture(ownershipFixture().records, { protectedAccounts: [fp("admin")] });
const remediationResults = await Promise.allSettled([
  ...
  runAddressOrganization({ manifest, runId, evidenceDir: "contract-evidence", ...drivers({ cleanup: existingProtectedOnly }) }),
]);
assert.deepEqual(remediationResults.map((item) => item.status === "fulfilled" ? "PASS" : item.reason?.code), ["PASS", "PASS", "PASS"]);
assert.deepEqual(remediationResults[2].value.cleanup.protectedAccounts.map((x) => x.id), ["admin"]);
checks.push("organization GET repetitions preserved", "imported audit targets owner", "only existing protected accounts fingerprinted");
...
const fabricatedProtected = cleanupFixture(ownershipFixture().records, { protectedAccounts: [fp("admin"), fp("cyhuh", false)] }); await expectCode("CLEANUP_INCOMPLETE", drivers({ cleanup: fabricatedProtected })); checks.push("missing protected account not fabricated");
