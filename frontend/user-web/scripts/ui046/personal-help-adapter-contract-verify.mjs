...
function networkFixture() { return [
  ...
  ...Array.from({ length: 5 }, () => ({ method: "GET", path: "/api/v1/workspace/profile", status: 200 })),
  ...Array.from({ length: 8 }, () => ({ method: "GET", path: "/api/v1/workspace/preferences", status: 200 })),
  ...Array.from({ length: 5 }, () => ({ method: "GET", path: "/api/v1/notifications/preferences", status: 200 })),
...
], profile: { matchesPreparedIdentity: true, readOnly: true, fieldCount: 5 }, general: { locale: "ko-KR", timezone: "Asia/Tokyo", startPage: "files", version: 1, rereadMatches: true }, notifications: { enabled: true, quietHoursEnabled: true, quietHoursStart: "22:00", quietHoursEnd: "07:00", mail: { enabled: true, importantOnly: true }, rereadMatches: true }, security: { clientMismatchRequestCount: 0, changeRequestCount: 1, reloginWithNewCredential: true, sensitiveEvidenceFields: 0 }, modules: { visible: ["mail", "approval", "calendar"], foreignNavigationCount: 0 }, help: { allCount: 6, errorSearchCount: 4, errorCategoryCount: 4, emptyCount: 0, selectedDocumentVisible: true, mutationCount: 0, searchEvidencePersisted: false }, visual: { status: "PASS", viewport: "1920x1080", bodyPx: 12, screenTitlePx: 16, sectionTitlePx: 14, helperPx: 10, infoTooltip: true, mismatches: [] }, network: networkFixture(), mutationOwnership: [
...
function auditRows() { return ["workspace.profile.viewed", "workspace.profile.viewed", "workspace.profile.viewed", "workspace.profile.viewed", "workspace.profile.viewed", ...Array.from({ length: 8 }, () => "workspace.preferences.viewed"), "workspace.preferences.updated", "workspace.help.viewed", "workspace.help.viewed", "workspace.help.viewed", "workspace.help.viewed", "auth.password.changed"].map((event) => ({ event, actorId: ids.user, targetId: ids.user, ownerRunId: runId, reasonSafe: true })); }
function dbFixture(records, overrides = {}) { return { rows: records.filter((row) => ["workspace_preference", "notification_preference"].includes(row.kind)).map((row) => ({ kind: row.kind, id: row.id, userId: ids.user, ownerRunId: runId })), preference: { userId: ids.user, companyId, locale: "ko-KR", timezone: "Asia/Tokyo", startPage: "files", version: 1 }, notificationPreference: { userId: ids.user, enabled: true, quietHoursEnabled: true, quietHoursStart: "22:00", quietHoursEnd: "07:00", mail: { enabled: true, importantOnly: true } }, helpFingerprint: { before: "help-published-fp", after: "help-published-fp", beforeCount: 6, afterCount: 6 }, audits: auditRows(), notificationAudits: Array.from({ length: 2 }, () => ({ event: "notification.preferences.updated", actorId: ids.user, targetId: ids.user, ownerRunId: runId, reasonSafe: true })), existingRowChanges: 0, ...overrides }; }
...
const mismatchNetwork = browserFixture(); mismatchNetwork.security.clientMismatchRequestCount = 1; await expectCode("PASSWORD_FLOW_INVALID", drivers({ browser: mismatchNetwork })); checks.push("client mismatch no network");
const noRelogin = browserFixture(); noRelogin.security.reloginWithNewCredential = false; await expectCode("PASSWORD_FLOW_INVALID", drivers({ browser: noRelogin })); checks.push("password relogin");
...
const visualGap = browserFixture(); visualGap.visual = { status: "GAP", viewport: "1920x1080", bodyPx: 12, screenTitlePx: 16, sectionTitlePx: 16, helperPx: 10, infoTooltip: true, mismatches: ["sectionTitlePx:16!=14"] }; const visualGapResult = await runPersonalHelp({ manifest, runId, evidenceDir: "contract-evidence", cleanupApproved: true, ...drivers({ browser: visualGap }) }); assert.equal(visualGapResult.visual.status, "GAP"); checks.push("visual gap preserved separately");
...
const secret = browserFixture(); secret.security.token = "forbidden"; await expectCode("SENSITIVE_FIELD_REJECTED", drivers({ browser: secret })); checks.push("secret field rejected");
...
const wrongNotification = dbFixture([...ownershipFixture().records, ...browserFixture().createdRecords]); wrongNotification.notificationPreference.quietHoursEnd = "06:30"; await expectCode("DB_NOTIFICATION_INVALID", drivers({ db: wrongNotification })); checks.push("DB notification state");
