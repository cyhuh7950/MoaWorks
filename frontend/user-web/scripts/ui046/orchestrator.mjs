...
async function executeArea() {
  assertRunId();
  if (!["home-search-notification", "mail", "approval", "calendar", "messenger"].includes(areaId)) throw errorWithCode("AREA_NOT_READY");
  const directory = safeEvidenceDir();
  const drivers = await loadRuntimeDrivers();
  const runners = { "home-search-notification": runHomeSearchNotification, mail: runMail, approval: runApproval, calendar: runCalendar, messenger: runMessenger };
